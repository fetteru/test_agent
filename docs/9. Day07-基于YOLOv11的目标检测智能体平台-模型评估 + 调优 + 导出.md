    ...
    if results.box.ap:
        for i, ap50 in enumerate(results.box.ap50):
            ...
            report["per_class"][class_name] = {
                ...
                "instances": int(results.box.np[i]) if hasattr(results.box, 'np') else None,
            }
...
│    ├── 主版本: 模型架构变更（如 yolov11n → yolov11s）             │
...
```bash
# ── 7.4.1 模型评估 ──
curl -X POST http://localhost:8000/api/training/validate/1 \
...
#   },
#   "per_class": {
#     "aircraft": {"ap50": 0.912, "ap50_95": 0.601},
#     "oiltank": {"ap50": 0.798, "ap50_95": 0.489}
#   },
#   "model_version_id": 1,
...
# ── 7.4.2 模型导出 ──
curl -X POST http://localhost:8000/api/training/export/1 \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "version": "v1.0.0",
    "description": "首次训练，基线模型",
    "set_default": true,
    "upload_minio": true
  }'

# ── 7.4.3 下载模型 ──
curl -X GET http://localhost:8000/api/training/download/1 \
  -H "Authorization: Bearer $TOKEN" \
  -o best_model.pt
...
async function validateModel() {
  ...
  try {
    ...
    ElMessage.success(`评估完成: mAP50=${(res.data.overall.map50 * 100).toFixed(1)}%`)
  } catch (e) {
...
async function exportModel() {
  ...
  try {
    const taskId = selectedTask.value.id || selectedTask.value.task?.id
    const res = await request.post(`/api/training/export/${taskId}`, exportForm.value)
    ElMessage.success(res.data.message || '模型导出成功')
